...
export function CharacterDetail({ character }: CharacterDetailProps) {
  ...
  const handleDelete = () => {
    if (confirm(`Bạn có chắc muốn xoá nhân vật "${character.name}"?`)) {
  ...
  const handleExportImage = async (imageUrl: string, name: string) => {
    try {
      ...
      toast.success(`Xuất ${name}.png thành công`);
    } catch (err) {
      ...
      <ScrollArea className="flex-1">
        <div className="p-3 space-y-4 pb-32">
            ...
            <div className="space-y-1">
              ...
              {isEditingNotes ? (
                ...
                <p className="text-xs bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800 rounded p-2 text-indigo-800 dark:text-indigo-200">
                  {character.notes || 'Nhấn chỉnh sửa để thêm ghi chú...'}
                </p>
              )}
            </div>
          ...
          <div className="text-xs text-muted-foreground space-y-1">
            <p>💡 Kéo thả ảnh nhân vật vào panel Đạo diễn AI để sử dụng</p>
          </div>
        </div>
      </ScrollArea>
